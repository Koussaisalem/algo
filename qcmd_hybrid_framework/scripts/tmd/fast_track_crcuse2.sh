#!/bin/bash
###############################################################################
# Fast-Track Validation for CrCuSe₂ (Hetero-Metallic TMD Alloy!)
# 
...
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║                                                              ║${NC}"
echo -e "${BLUE}║      🚀 Fast-Track Validation for CrCuSe₂ Alloy            ║${NC}"
echo -e "${BLUE}║         (First Hetero-Metallic TMD!)                        ║${NC}"
echo -e "${BLUE}║                                                              ║${NC}"
...
if [ ! -f "dft_validation/priority/CrCuSe2_candidate.xyz" ]; then
    echo -e "${RED}❌ ERROR: CrCuSe2_candidate.xyz not found!${NC}"
    echo "Expected location: dft_validation/priority/CrCuSe2_candidate.xyz"
    exit 1
fi

echo "Running xTB pre-relaxation..."
python scripts/tmd/prerelax_crcuse2.py

PHASE1_END=$(date +%s)
PHASE1_DURATION=$((PHASE1_END - PHASE1_START))

if [ ! -f "dft_validation/priority/CrCuSe2_prerelaxed.xyz" ]; then
...
echo -e "   Output: dft_validation/priority/CrCuSe2_prerelaxed.xyz"
echo ""
...
python scripts/tmd/05_validate_with_dft.py \
    dft_validation/priority/CrCuSe2_prerelaxed.xyz \
    --mode fast \
    --single-point \
    --force
...
echo "   dft_validation/results/CrCuSe2_prerelaxed_results.json"
echo ""

# Check results file and extract key metrics
RESULTS_FILE="dft_validation/results/CrCuSe2_prerelaxed_results.json"
...
if results_file.exists():
    ...
    if max_force < 1.5:
        ...
        print("      → This hetero-metallic alloy is REAL!")
...
echo "      dft_validation/priority/CrCuSe2_prerelaxed.xyz \\"
echo "      --mode production"
echo ""
echo "If NO-GO SIGNAL (❌):"
echo "  Structure is not viable. Consider:"
echo "  - Trying VTe₂ (magnetic TMD, known to be stable)"
echo "  - Analyzing what went wrong with generation"
echo ""
